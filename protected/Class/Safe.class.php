...
class sqlsafe{
  ...
  {
    $this->webscan_referer = empty($_SERVER['PHP_SELF']) ? array() : array('PHP_SELF'=>$_SERVER['PHP_SELF']);
  ...
  public function webscan_StopAttack($StrFiltKey,$StrFiltValue,$ArrFiltReq) {
    if(is_array($StrFiltValue)) {
      $StrFiltValue=implode($StrFiltValue);
    }
    if (preg_match("/".$ArrFiltReq."/is",$StrFiltValue)==1){
      error($StrFiltKey,$StrFiltValue,1);
    }
  ...
  public function webscan(){
    foreach($_GET as $key=>$value){
      $this->webscan_StopAttack($key,$value,$this->getfilter);
    }
    foreach($_POST as $key=>$value){
      $this->webscan_StopAttack($key,$value,$this->postfilter);
    }
    foreach($_COOKIE as $key=>$value){
      $this->webscan_StopAttack($key,$value,$this->cookiefilter);
    }
  ...
  public function arrscan($arr){
    foreach($arr as $key=>$value){
      $this->webscan_StopAttack($key,$value,$this->postfilter);
    }
  }

  public function scan($key,$value){
      $this->webscan_StopAttack($key,$value,$this->postfilter);
  }
